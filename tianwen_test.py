...
end_time = ts.utc(datetime.utcnow().year + 5)
...
def find_retrograde(planet, name):
    ...
    while current_time < end_time:
        ...
        if not in_retrograde and current_longitude < previous_longitude:
            ...
            print(f"{name} 逆行开始于 {retro_start.strftime('%Y-%m-%d %H:%M:%S')} CST, 黄道值: {retro_start_longitude:.2f}°")
            ...
            print(f"{name}逆行结束于 {retro_end.strftime('%Y-%m-%d %H:%M:%S')} CST, 黄道值: {retro_end_longitude:.2f}°")
            print(description)
            # 添加逆行事件到事件列表
            event = Event()
...
# 计算火星、土星、木星逆行数据
find_retrograde(eph['MARS BARYCENTER'], "火星")
find_retrograde(eph['SATURN BARYCENTER'], "土星")
find_retrograde(eph['JUPITER BARYCENTER'], "木星")
